"""Sonido de la Story B2B para ferreterías (7 s). Los tiempos coinciden con b2b.html.
...
m.place(snap(), 1.2, 0.35, pan=0.2)          # cambio de titular

# beat + bajo hasta la cortina final
kicks = np.arange(0.2 + BEAT, 3.8, BEAT)
for tk in kicks:
    if abs(tk - 2.2) > 1e-6:                 # en 2.2 va el golpe de transición
        m.place(kick(), tk, 0.75)
for th in np.arange(0.4, 3.9, BEAT / 2):
    off = abs(((th - 0.2) / (BEAT / 2)) % 2 - 1) < 1e-6
    m.place(hat(), th, 0.2 if off else 0.07, pan=0.35 if off else -0.35)
m.bass_line([82.41, 82.41, 164.8, 82.41, 98.0, 82.41, 110.0, 123.5], 0.6, 3.8, BEAT / 2, kicks, gain=0.32)

# transición a la escena 2
m.place(whoosh(0.24, 800, 7000, "rise"), 1.96, 0.6, pan=-0.3)
m.place(impact(0.8), 2.2, 0.75)

# tarjetas: entran en cada negra, contador del +25
for t, pan in ((2.4, -0.3), (2.8, 0.3), (3.2, -0.3)):
...
for n in range(1, 26):
    ...
    m.place(tick(0.02), 2.46 + p * 0.45, 0.08, pan=-0.2)

# subida + cortina blanca + golpe de marca
m.place(whoosh(0.56, 300, 9000, "rise"), 3.64, 0.55)
m.place(whoosh(0.26, 1500, 8000), 3.94, 0.5, pan=0.4)
m.place(impact(1.8), 4.2, 1.0, verb=0.3)
m.place(chord_stab([164.8, 246.9, 329.6, 415.3, 493.9, 659.3], 2.6), 4.22, 0.55, verb=0.5)
...
m.place(pad([164.8, 246.9, 329.6, 415.3], 2.8), 4.2, 0.35, verb=0.3)
